...
PALATTE = {
    v: color for v, color in zip(BASELINE_NAME_MAP.values(), sns.color_palette()[:len(BASELINE_NAME_MAP)])
}

ATTACK_NAME_MAP = {
    'gps_stealthy_attack_default': r'$SA_{GPS}$',
...
if __name__ == "__main__":
    ...
    csv_path = 'data/evaluation_reports/Stealthy Attacks - 04.14/full_detection_reports.csv'
    ...
    # df = df.loc[df.loc[:, 'ATK_APPLY_TYPE'] == AttackType.Gyroscope]
    ...
    with sns.axes_style("whitegrid"), sns.plotting_context(font_scale=1.6):
        ...
        for row_idx, row in enumerate(plot.axes):
            for col_idx, ax in enumerate(row):
                ...
                ax.set_yscale('log')
                if row_idx == GPS_ROW_IDX:
                    ax.xaxis.label.set_text(r"Absolute Residual (meter)")
                    ax.set_ylim(90, 50.0 * SEC_TO_MS)

                    ax.yaxis.set_major_formatter(ScalarFormatter())
                    ticks = np.asarray([100, 1000, 10000, 20000, 30000])
                    ax.set_yticks(ticks, ['100', '1000', '10000', '20000', NOT_APPLICABLE_LABEL])
                    ax.axhline(20000, color="red", linewidth=1)
                    if col_idx == 0:
    ...
    plot.tight_layout(pad=0.5, rect=[0, 0.05, 1.0, 1.0])
    plot.figure.subplots_adjust(wspace=0.01)
    plot.figure.show()

    with PdfPages('Stealthy Time to Detect.pdf') as pdf_pages:
