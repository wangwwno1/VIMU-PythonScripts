...
if __name__ == "__main__":
    ...
    SEC_TO_MS = 1e3
    GYRO_TPR_TTD_THRESHOLD_MS = 1 * SEC_TO_MS
    ...
    tp_data = get_true_positive_rate(df, GYRO_TPR_TTD_THRESHOLD_MS)
    tp_data = tp_data.loc[tp_data[DEVIATION_LABEL] < 0.601]
    with sns.axes_style("whitegrid"), sns.plotting_context(font_scale=1.6):
        ...
        tpr_graph = sns.FacetGrid(
            ...
            row='Attack', row_order=GYRO_ATTACK_ORDER,
            **grid_kwargs
        )

        cbar_ax = tpr_graph.fig.add_axes([0.125, 0.04, .805, .01])
        ...
        for row_idx, row in enumerate(tpr_graph.axes):
            for col_idx, ax in enumerate(np.atleast_1d(row)):
                # Gyro Attack
                x_label = "Gyro Deviation (rad/s)"
                ax.set_xlabel(None)
                if row_idx == max_row_idx:
                    ax.set_xlabel(x_label)
        tpr_graph.tight_layout(pad=0.5, rect=[0, 0.05, 1.0, 1.0])
        tpr_graph.figure.subplots_adjust(wspace=0.0, hspace=0.01)
        tpr_graph.figure.show()

        with PdfPages('TPR_heatmap Overt Gyro Attacks - 4x2.pdf') as pdf_pages:
